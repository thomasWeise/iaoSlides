...
%
\blInstitute{%
\blUni{Hefei University, South Campus 2}{合肥学院 南艳湖校区/南2区}\\%
\blFaculty{Faculty of Computer Science and Technology}{计算机科学与技术系}\\%
\blGroup{Institute of Applied Optimization}{应用优化研究所}\\%
\blAddressA{230601}{Shushan District, Hefei, Anhui, China}{中国 安徽省 合肥市 蜀山区}\\%
...
\lstinputlisting[language=TeX]{./styles/pack_units.sty}%
}%
\end{scaledBox}%
\end{listingBlock}%
\end{lstlisting}%
...
\item<2-> Any frame containing a listing must specify \codeil{[containsverbatim]}, i.e., \codeBox{\texttt{{\textbackslash}begin\{frame\}[containsverbatim]\%}}%
\item<3-> There then should not be a percent mark after the end of the frame, i.e., write \codeBox{\texttt{{\textbackslash}end\{frame\}}} instead of \codeBox{\texttt{{\textbackslash}end\{frame\}\%}}
\item<4-> If and only if the frame uses overlays (e.g., \codeBox{\texttt{only}}, \codeBox{\texttt{uncover}}, or any \codeBox{\texttt{<>}} form of commands, it must also define \codeBox{\texttt{fragile}}, i.e.,  \codeBox{\texttt{{\textbackslash}begin\{frame\}[containsverbatim,fragile]\%}}%
\item<5-> You can put \codeBox{\texttt{lstinputlisting}}s into \codeBox{\texttt{parboxes}} which you can resize via \codeBox{\texttt{scaledBox}} or \codeBox{\texttt{resizebox}}%
\item<6-> Plain listings \codeBox{\texttt{lstlisting}} must go into \codeBox{\texttt{minipage}}s to be resized via \codeBox{\texttt{scaledBox}}% 
\end{itemize}%
\end{frame}%
%
\section{Including Algorithms}%
%
\begin{frame}%
\frametitle{An Algorithm}%
%
\putAlgorithm{x}{\textit{exhaustiveEnumeration}(f,\underline{f})}%
{%
\aIn{f}{the objective/fitness function}%
\aIn{\underline{f}}{the lowest possible objective value, $-\infty$ if unknown}%
\aVar{x}{the current candidate solution}%
\aOut{\xi}{the search result}%
}%
{%
\aAssign{\xi}{\textnormal{first element in search space}}%
\aAssign{t}{1}%
\aWhile{ \textnormal{not all solutions checked} \land \left(f(\xi)>\underline{f}\right) }{%
\aAssign{x}{\textnormal{next element in search space}}%
\alIf{f(x)>f(\xi)}{\aAssign{\xi}{x}}%
}%
\aRet{\xi}%%
}%
\end{frame}%
\begin{frame}%
\frametitle{A Zoomed Algorithm}%
%
\putZoomedAlgorithm{0.6}{x}{\textit{exhaustiveEnumeration}(f,\underline{f})}%
{%
\aIn{f}{the objective/fitness function}%
\aIn{\underline{f}}{the lowest possible objective value, $-\infty$ if unknown}%
\aVar{x}{the current candidate solution}%
